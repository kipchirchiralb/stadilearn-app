...
export async function issueOtp(opts: {
  ...
  const [recent] = await appDb.query<{ n: number }>(
    `SELECT COUNT(*) AS n FROM otp_codes
     WHERE email = ? AND purpose = ? AND created_at > UTC_TIMESTAMP(3) - INTERVAL 60 SECOND`,
    [email, opts.purpose],
  );
  if (Number(recent?.n) > 0) return { queued: false };

  const [hourly] = await appDb.query<{ n: number }>(
    `SELECT COUNT(*) AS n FROM otp_codes
     WHERE email = ? AND created_at > UTC_TIMESTAMP(3) - INTERVAL 1 HOUR`,
    [email],
  );
  if (Number(hourly?.n) >= 8) return { queued: false };
  ...
  const inserted = await appDb.execute(
    `INSERT INTO otp_codes (email, user_id, purpose, code_hash, expires_at, request_ip_hash)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [email, user.id, opts.purpose, hmacOtp(code), expiresAt, opts.ip ? sha256(opts.ip) : null],
  );
...
export async function consumeOtp(email: string, code: string, purpose?: OtpPurpose) {
  ...
  const [row] = await appDb.query<{
    ...
    purpose ? [normalised, purpose] : [normalised],
  );
  ...
  await appDb.execute("UPDATE otp_codes SET consumed_at = UTC_TIMESTAMP(3), attempts = attempts + 1 WHERE id = ?", [
    row.id,
  ]);
